...
//
// The app uses `lucide-svelte`; this is a separate Astro project, so only the
// node data for the icons this site actually draws is copied here instead of
// pulling the whole package in. Same library, same 24x24 grid, same stroke
// weight, so the two surfaces render one icon set.
//
// Lucide is ISC-licensed. `github` is authored here because Lucide dropped
// brand icons.
...
/** Brand marks are solid shapes, not stroked line art, so they paint with a fill. */
